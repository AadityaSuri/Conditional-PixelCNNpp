...
def train_or_test(model, data_loader, optimizer, loss_op, device, args, epoch, mode = 'training'):
    if mode == 'training':
        acc_check_interval = 50
        model.train()
    else:
        model.eval()
        acc_check_interval = 20
    ...
    for batch_idx, (model_input, class_label) in enumerate(tqdm(data_loader)):

        model_input = model_input.to(device)

        target_class = [my_bidict[item] for item in class_label]
        target_class = torch.tensor(target_class, dtype=torch.int64).to(device)

        model_output = model(model_input, class_label=target_class)
        gen_loss = loss_op(model_input, model_output, 'generation')


        # to log classification accuracy during training
        if batch_idx % acc_check_interval == 0:
            class_losses = []
            for _, i in my_bidict.items():
                dummy_label = torch.full((len(class_label),), i, dtype=torch.int64).to(device)
                dummy_output = model(model_input, class_label=dummy_label)
                class_loss = loss_op(model_input, dummy_output, 'classification')
                class_losses.append(class_loss)

            class_losses = torch.stack(class_losses)
            _, predicted_class = class_losses.min(dim=0)

            correct_predictions = (target_class == predicted_class)
            accuracy = correct_predictions.float().mean()
            accuracies.append(accuracy)
            accuracy_count += 1


        loss_tracker.update(gen_loss.item()/deno)
        if mode == 'training':
            optimizer.zero_grad()
            gen_loss.backward()
            optimizer.step()
    ...
    if args.en_wandb:
        ...
        wandb.log({mode + "-Average-Accuracy" : mean_accuracy})
...
if __name__ == '__main__':
    ...
    loss_op   = lambda real, fake, mode : discretized_mix_logistic_loss(real, fake, mode)
    ...
    if args.load_params:
        ...
        checkpoint = torch.load(args.load_params, map_location=device)
    ...
    for epoch in tqdm(range(args.max_epochs)):
        ...
        if (epoch + 1) % args.save_interval == 0: 
            ...
            torch.save(checkpoint, 'models/{}_{}.pth'.format(model_name, epoch))

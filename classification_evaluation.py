...
def get_label(model, model_input, device):
    ...
    # calculate loss for each class for all images in the batch
    ...
    class_losses = torch.stack(class_losses).to(device)
    _, predicted_class = class_losses.min(dim=0)  # pick the class with the lowest loss

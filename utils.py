...
def discretized_mix_logistic_loss(x, l, mode = 'generation'):
...
def sample(model, sample_batch_size, obs, sample_op):
    ...
    with torch.no_grad():
        ...
        quarter_size = sample_batch_size // 4
        class_labels = torch.cat([torch.full((quarter_size,), i) for i in range(4)]).to(data.device)

        for i in range(obs[1]):
            print(i)
            for j in range(obs[2]):
                data_v = data
                out   = model(data_v, sample=True, class_label=class_labels)
